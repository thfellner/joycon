    ...
    mapButtonsForMode(buttons,mode) {
        ...
        if (!isEquivalent(this.lastPressedButtons, minorChangeIgnoredButtons)) {
            sockets.forEach(async (socket) => {
                ...
                await this.sendSubcommand(0x10, Buffer.from([0x0, 0x0, 0x1, 0x0, 0x0, 0x64, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0]))
                //0x10, 0x0, 0x0, 0x0, 0x0, 0x1, 0x64, 0x0, 0x0, 0x1
            });
            ...
            //this.hidDevice.write([0x10, 0x0, 0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x1]);

        }

        this.lastPressedButtons = minorChangeIgnoredButtons;

        let newButtonsBuffer = newButtons;

        if (this.type === this.LEFT) {
            newButtonsBuffer.leftStickHorz = newButtons.leftStickVert;
            newButtonsBuffer.leftStickVert = newButtons.leftStickHorz;
        } else if (this.type === this.RIGHT) {
            newButtonsBuffer.rightStickHorz = newButtons.rightStickVert * -1;
            newButtonsBuffer.rightStickVert = newButtons.rightStickHorz * -1;
        }
